  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-medium text-gray-900">{t('contactsExtractor.personal.title')}</h3>
          <p className="text-sm text-gray-500">
            {t('contactsExtractor.personal.selectedSummary', {
              selected: selectedJids.size,
              total: chats.length,
            })}
          </p>
        </div>
      ...
      {chats.length > 0 && (
        <div className="relative mb-4">
          ...
          <input
            type="text"
            placeholder={t('contactsExtractor.personal.searchPlaceholder')}
            value={search}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="mt-2 font-medium">{t('contactsExtractor.personal.empty')}</p>
          <p className="text-sm mt-1">{t('contactsExtractor.personal.emptyHint')}</p>
          <p className="text-sm">{t('contactsExtractor.personal.emptyHint2')}</p>
        </div>
      ) : filteredChats.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          <p className="font-medium">{t('contactsExtractor.personal.noMatch')}</p>
          <p className="text-sm mt-1">{t('contactsExtractor.personal.noMatchHint')}</p>
        </div>
      ) : (
      ...
      {filteredChats.length > 0 && (
        <div className="mt-3 pt-3 border-t text-xs text-gray-400 text-center">
          {t('contactsExtractor.personal.showing', { shown: filteredChats.length, total: chats.length })}
        </div>
      )}
    </div>
  );
